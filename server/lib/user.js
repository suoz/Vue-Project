...
const findOneLean = (key, value, password) => {
  return new Promise((resolve, reject) => {
    userModel.findOne({ [key]: value }, { password }, (err, doc) => {
      if (err) {
        reject(err)
...
exports.saveUserSocketId = async (username, socketId) => {
  const conditions = {username}
  const updates = {$set: {socketId}}
  await userModel.update(conditions, updates, (err) => {
    if (err) {
      throw err
    } else {
      console.log('success')
...
exports.getUserSocketId = async (userId) => {
  let doc = await findOneLean('userId', userId, 0)
  return doc
}
